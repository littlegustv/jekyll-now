...
var Fire = {
	square: function (entity, points) {
      ...
      var flame = entity.layer.add(Object.create(Sprite).init(Resources.fire)).set({x: points[i].x, y: points[i].y, color: "orange", w: TILESIZE, h: TILESIZE, z: entity.z - 1, family: 'fire'});
  ...
  if (shapes.length <= 0) console.log('no matching size');
    ...
    if (delta > this.threshold) {
      ...
      if (Fire[shapes[0].name]) {
        Fire[shapes[0].name](this.entity, this.points);
      }      
    }    
  }
}
  ...
  ctx.lineWidth = 2;
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      ...
      for (var x = p0.x; (sign(dx) == 1 ? x <= p1.x : x >= p1.x); (sign(dx) == 1 ? x++ : x--)) {
        ...
        ctx.fillRect(pout.x, pout.y, TILESIZE, TILESIZE);
        ctx.drawImage(Resources.icon.image, pout.x, pout.y);
        error += derr;
        while (error >= 0.5) {
          y = y + sign(dy) * 1;
          error = error - 1;
