...
function spawn(layer, key, player) {
	...
	switch (key) {
		...
		case 2:
			...
			enemy.angle = angle(enemy.x, enemy.y, gameWorld.width / 2, gameWorld.height / 2 + 60);
			enemy.velocity = {x: Math.cos(enemy.angle) * 50, y: 50 * Math.sin(enemy.angle)}; 	
			enemy.addBehavior(Asteroid, {target: {x: gameWorld.width / 2, y: gameWorld.height / 2 + 60}, radius: 70});
		break;
		case 3:
